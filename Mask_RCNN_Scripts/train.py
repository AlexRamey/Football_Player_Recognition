...
if (len(sys.argv) != 3):
    sys.exit("2 cmd line arguments are required: train set [svhn|football] and test set [svhn|football]")

if sys.argv[1] != 'svhn' and sys.argv[1] != 'football':
    sys.exit("Invalid train arg. Got " + sys.argv[1] + ", expected svhn or football")
if sys.argv[2] != 'svhn' and sys.argv[2] != 'football':
    sys.exit("Invalid test arg. Got " + sys.argv[2] + ", expected svhn or football")
